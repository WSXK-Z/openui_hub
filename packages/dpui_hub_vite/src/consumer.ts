/**
 * dpui_hub 使用者（消费端）Vite 插件：构建期远程组件接入。
 *
 * 用法（宿主 vite.config）：
 * ```ts
...
async function fetchCached(cwd: string, url: string, context: { warn: (msg: string) => void }): Promise<string> {
  ...
  if (!res.ok) {
    throw new Error(`dpui_hub: 拉取失败 ${url} → HTTP ${res.status}`)
  }
  ...
  try {
    ...
    context.warn(`dpui_hub: 缓存写入失败（不影响本次构建）: ${file}`)
  }
  return text
}
...
function resolveEntryUrls(
  ...
  if (!urls) {
    throw new Error(
      `dpui_hub: 无法确定 ${name} 的下载地址。lock 使用相对路径，需要 registry：` +
