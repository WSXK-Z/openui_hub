...
import type { DpProps, DpSize, DpThemeProps, DpType } from '../types'

export interface UseDpPropsOptions<Comp extends object> {
  /** comp（业务数据）默认值，需为 Comp 全部字段提供默认 */
  compDefaults?: Partial<Comp>
  /** theme（外观数据）默认值 */
  themeDefaults?: Partial<DpThemeProps>
}

/** 合并默认值后的完整样式数据（关键字段已解析为必填） */
export interface ResolvedDpStyle {
...
export interface UseDpPropsReturn<Comp extends object> {
  /** 合并默认值后的业务数据（响应式）。Comp 字段定义为必填，运行时由 compDefaults 补齐 */
  comp: ComputedRef<Comp>
  /** 合并默认值后的样式数据（响应式） */
  style: ComputedRef<ResolvedDpStyle>
 ...
 *   自动合并透传的 class/style、theme 的 class/style/cssVars（写入 `--dp-*`）。
 * - 组件需设置 `defineOptions({ inheritAttrs: false })` 避免自动透传重复。
 */
export function useDpProps<Comp extends object>(
  props: DpProps<Comp>,
  options: UseDpPropsOptions<Comp> = {},
): UseDpPropsReturn<Comp> {
  const attrs = useAttrs()

  const comp = computed<Comp>(() => ({ ...options.compDefaults, ...props.comp }) as Comp)

  const style = computed<ResolvedDpStyle>(() => ({
    ...
    ghost: props.theme?.ghost ?? options.themeDefaults?.ghost ?? false,
    icon: props.theme?.icon ?? options.themeDefaults?.icon ?? false,
...
function normalizeCssVars(vars: Record<string, string | number>): Record<string, string> {
  ...
  for (const [key, value] of Object.entries(vars)) {
    result[key.startsWith('--') ? key : `--dp-${key}`] = String(value)
  }
  return result
}
