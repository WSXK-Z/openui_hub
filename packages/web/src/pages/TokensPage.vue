...
const deleteTarget = ref<HubToken | null>(null)
...
async function toggle(token: HubToken) {
  busyId.value = token.id
  error.value = ''
  notice.value = ''
  try {
    await setTokenEnabled(token.id, !!token.revokedAt)
    await load()
    notice.value = `${token.name} 已${token.revokedAt ? '启用' : '停用'}`
  } catch (e) {
    report(e)
  } finally {
    busyId.value = null
    ...
    <table v-else class="mt-4 w-full border-collapse rounded border border-gray-200 bg-white text-sm">
      ...
      <tbody>
        <tr v-for="t in tokens" :key="t.id" class="border-b border-gray-100">
          ...
          <td class="px-3 py-2">
            <HubTag v-if="t.revokedAt" type="danger">已停用</HubTag>
            <HubTag v-else type="success">已启用</HubTag>
          </td>
          <td class="px-3 py-2">
            <button
              type="button"
              :disabled="busyId === t.id"
              class="rounded border border-gray-300 px-2 py-1 text-xs text-gray-600 hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-55"
              @click="toggle(t)"
            >
              {{ t.revokedAt ? '启用' : '停用' }}
            </button>
            <button
              type="button"
              :disabled="busyId === t.id"
              class="ml-2 rounded border border-red-200 px-2 py-1 text-xs text-red-600 hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-55"
        ...
        >
          ...
          <div class="mt-4 flex justify-end gap-2">
            ...
            <button
              ...
              @click="deleteTarget && deleteToken(deleteTarget.id).then(() => { deleteTarget = null; load(); notice = '令牌已删除' }).catch(report)"
            >
              删除
            </button>
          </div>
