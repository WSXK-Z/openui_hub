//! dpui —— openui_hub 本地终端工具（发布端 + 消费端）。
...
#[derive(Parser)]
#[command(name = "dpui", version, about = "openui_hub 组件分发工具（发布端 + 消费端）")]
struct Cli {
    #[command(subcommand)]
    cmd: Cmd,
}
...
/// 接入远程组件类型：
/// 1) 根目录建 `dpui.d.ts`（三斜杠引用 `@openui_hub/plugin_vite/remote`）；
...
async fn cmd_use_remote(pkg: &str, registry: &Option<String>, with_types: bool) -> Result<()> {
    ...
    println!("构建期接入：安装 @openui_hub/plugin_vite 并在 vite.config 添加 hubVite()；代码中 import {{ Button }} from 'dpui-hub:{base}'");
    Ok(())
}
