#!/usr/bin/env node
/**
 * oui 元包 bin shim：从本包 vendor/<os>-<arch>/ 选择对应平台二进制并转发执行。
 * 使 `npm i -g @openui_hub/cli` 后直接获得 `oui` 命令（单包内自带全部平台产物）。
 */
'use strict'
...
const binName = process.platform === 'win32' ? 'oui.exe' : 'oui'
const binPath = path.join(__dirname, '..', 'vendor', `${platformName}-${archName}`, binName)

if (!fs.existsSync(binPath)) {
  console.error(
    `oui: 本包缺少当前平台二进制 ${path.relative(process.cwd(), binPath)}。` +
