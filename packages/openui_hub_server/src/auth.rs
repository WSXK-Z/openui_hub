...
pub fn new_publish_token() -> String {
    format!("dpui_{}", rand_hex(20))
}

pub fn new_session_token() -> String {
    format!("dpui_sess_{}", rand_hex(20))
}
...
/// DELETE /api/tokens/{id} → 吊销（admin 或令牌所有者）
pub async fn delete_token(
    ...
    state.store.revoke_token(id).await.map_err(internal)?;
    Ok(Json(json!({ "ok": true })))
}
...
mod tests {
    ...
    async fn setup_login_and_status() {
        ...
        assert!(v["sessionToken"].as_str().unwrap().starts_with("dpui_sess_"));
        let (st, _) = call(
            &app,
            "POST",
            "/api/auth/login",
    ...
    async fn token_permission_matrix_and_publish() {
        ...
        for perms in [json!(["publish:@dp_ui"]), json!(["publish"]), json!(["read"])] {
            ...
            .await;
            assert_eq!(st, StatusCode::CREATED, "{v}");
            assert!(v["token"].as_str().unwrap().starts_with("dpui_"));
